...
import { X, Waves, Check, Loader2, Timer } from "lucide-react";
        ...
        >
          ...
          >
            ...
            <button
              onClick={dismiss}
              aria-label="إغلاق"
              className="absolute left-4 top-4 z-20 rounded-full p-1.5 text-white/60 hover:bg-white/10 hover:text-white"
            >
              <X className="h-5 w-5" />
            </button>
            ...
            <div className="relative p-6 sm:p-7">
              ...
              <p className="mt-1 text-center text-sm text-white/60">وش الأوقات المفضلة لكِ للمسبح؟</p>
              ...
              {results ? (
                ...
                <button onClick={vote} disabled={!choice || pending} className="btn-primary mt-5 w-full">
                  {pending ? <Loader2 className="h-5 w-5 animate-spin" /> : "أرسلي تصويتك"}
                </button>
              )}
            </div>
          </motion.div>
